...
<html>
    <head>
        ...
        <title>FPT University | Update</title>
                ...
                <form action="save" method="POST">
                    ...
                    <div class="body-result container" style="border: 2px solid gray; margin-top:20px; padding: 20px;  ">
                        <c:if test="${not empty requestScope.UPDATEERROR}">
                            <font style="color: red">${requestScope.UPDATEERROR}</font>
                        </c:if>
                        <div class="col-auto">
                            <input style="font-weight: bold;" type="text" id="inputPassword6" class="form-control" aria-describedby="passwordHelpInline"
                                   name="txtQuestion" value="${sessionScope.QUESNAME}"/>
                        </div>
                        <c:forEach items="${sessionScope.ANSWERLIST}" var="answer">
                            <input type="radio" <c:if test="${answer.isCorrect eq 1}">checked</c:if> name="rdoAnswer" value="${answer.content}" />
                            <input type="text" name="txtAnswer" value="${answer.content}" /><br/>
                        </c:forEach>
                            <select name="dropStatus">
                                <option <c:if test="${sessionScope.ACTIVE eq 1}">selected</c:if> >Active</option>
                                <option <c:if test="${sessionScope.ACTIVE eq 0}">selected</c:if> >Inactive</option>
                            </select>
